...
def handle_message(msg):
    ...
    if list(filter(message_text.startswith, commands_list)) != [] and chat_id == sender_from:
        ...
        if command_param[0] == "/getstatus":
            send_message(sender_from, f"Hi @{sender_username}, everything is fine!")
        
        # Get user id
        elif command_param[0] == "/getmyid":
            send_message(sender_from, f"@{sender_username}, your Telegram id: {sender_from}")
        ...
        # Get chat whitelist
            ...
            if sender_from in admins:
                ...
                if len(white_list) > 0:
                    ...
                    text_whitelist += f"\nIf you need *remove* user from whitelist, click the command /rem\_<user-id>\_{command_param[1]} in the list above."
                else:
                    text_whitelist += "\nWhitelist is empty.\n"
                text_whitelist += f"\nIf you want *add* user to whitelist for this group, send me command \n/add\_<user-id>\_{command_param[1]}"
    ...
    if str(sender_from) not in whitelist:
        ...
        if not regex.match(message_text):
            ...
            if autoreply_text == "":
                autoreply_text = f"Message from @{sender_username} was deleted"
            else:
                autoreply_text = autoreply_text.replace("<sender-username>", f"@{sender_username}")
                autoreply_text = autoreply_text.replace("<sender-id>", f"{str(sender_from)}")
            send_message(chat_id, autoreply_text)
